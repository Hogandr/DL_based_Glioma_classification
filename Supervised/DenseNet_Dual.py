...
from monai.transforms import (
    Compose, RandGaussianNoise, RandRotate, RandAxisFlip,
    RandAffine, OneOf, EnsureChannelFirst
)
from Helper.Helper_Dual import *
...
def run(rank: int, world_size: int, cfg: dict) -> None:
    ...
    # Create a 3D Dual_DenseNet model from MONAI, configured for binary classification
    model = Dual_DenseNet(spatial_dims=3,
                          in_channels=1,
                          out_channels=1,
                          tabular_input_dim=6,
                          init_features=64,
                          ...
                          norm='batch',
                          dropout_prob=cfg['dropout']).to(rank)

    # Checkpoint loading logic: resume training if model file exists
    ...
    if os.path.exists(path):
        ...
        start_epoch = 1  # Start from scratch if no checkpoint is found

    # Wrap the model in DistributedDataParallel (DDP) for multi-GPU training
    ...
    # Construct the path to the Excel file containing data splits
    path_splits = cfg["path_splits"]
    path_splits = path_splits + "_" + str(cfg["SPLIT"]) + ".xlsx"

    # Load data for training, validation, and testing from different sheets
    ...
    # Extract image paths (or image data) from each dataframe using the specified modality
    ...
    # Extract target variables and convert them to float64 numpy arrays
    # The labels include clinical and spatial features such as Status, Gender, Age, Tumor Size, and coordinates (y, x, z)
    train_labels = dataframe_train[
        ["Status", "Gender (1 Female/0 Male)", "Age at FET", "Tumor_Size", "y", "x", "z"]].to_numpy(dtype=np.float64)
    validation_labels = dataframe_validation[
        ["Status", "Gender (1 Female/0 Male)", "Age at FET", "Tumor_Size", "y", "x", "z"]].to_numpy(dtype=np.float64)
    test_labels = dataframe_test[
        ["Status", "Gender (1 Female/0 Male)", "Age at FET", "Tumor_Size", "y", "x", "z"]].to_numpy(dtype=np.float64)
...
def train(
    ...
    for ep in range(start_epoch, epochs + 1):
        ...
        for x, y in tqdm(train_loader, leave=False):
            # Print the original target tensor (y)
            print(y)
            print("y_before:", y.shape)

            # Separate the tabular input features (excluding the target variable) from y
            x_tabular = y[:, 1:]

            # Move tabular data to the correct device (rank) and convert to float
            x_tabular = x_tabular.to(rank)
            x_tabular = x_tabular.float()

            # Extract the actual target variable (e.g., classification label) as the first column
            y = y[:, 0]
            print(y)
            ...
            # Move image data and target variable to the correct device
            x = x.to(rank)
            y = y.to(rank)

            # Add a singleton dimension to y to match output shape (e.g., for BCE loss)
            y = y.unsqueeze_(1)

            # Forward pass through the model using image and tabular input
            y_hat = model(x, x_tabular)
            ...
            # Update the training loss metric with the current batch loss
            metric_train_loss(batch_loss)

            # Prepare the target for metric computation
            y_metric = y.float().squeeze(1)

            # Apply sigmoid to model outputs and prepare for metric computation
            y_hat_metric = torch.sigmoid(y_hat).squeeze(1)
            y_hat_metric = torch.tensor(y_hat_metric).to(rank)

            # Update metrics: accuracy, AUC, and F1 score
        ...
        for x, y in tqdm(validation_loader, leave=False):
            # Extract tabular features (excluding the target) from y
            x_tabular = y[:, 1:]
            x_tabular = x_tabular.to(rank).float()  # Move to device and convert to float

            # Extract the actual target variable
            y = y[:, 0]
            x = x.to(rank)  # Move image input to device
            y = y.to(rank)  # Move target to device

            # Disable gradient calculation for validation (saves memory and computation)
            with torch.no_grad():
                # Adjust target shape to match model output shape
                y = y.unsqueeze_(1)
            ...
            # Prepare targets and predictions for metric evaluation
            y_metric = y.float().squeeze(1)  # Convert target to float and remove extra dim
            y_hat_metric = torch.sigmoid(y_hat).squeeze(1)  # Apply sigmoid to model output
            y_hat_metric = torch.tensor(y_hat_metric).to(rank)  # Ensure it's on the correct device

            # Update validation metrics: accuracy, AUC, and F1 score
        ...
        for x, y in tqdm(test_loader, leave=False):
            # Extract tabular features (excluding the target column)
            x_tabular = y[:, 1:]
            x_tabular = x_tabular.to(rank).float()  # Move to the correct device and convert to float
            ...
            with torch.no_grad():
                # Add a singleton dimension to match model output shape
                y = y.unsqueeze_(1)
            ...
            # Prepare predictions and true labels for metric computation
            y_metric = y.float().squeeze(1)  # Convert to float and remove extra dimension
            y_hat_metric = torch.sigmoid(y_hat).squeeze(1)  # Apply sigmoid and squeeze output
            y_hat_metric = torch.tensor(y_hat_metric).to(rank)  # Ensure tensor is on the correct device

            # Update test metrics: accuracy, AUC, and F1 score
