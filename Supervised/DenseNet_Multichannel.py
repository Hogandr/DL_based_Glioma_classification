...
def run(rank: int, world_size: int, cfg: dict) -> None:
    ...
    # Create a 3D DenseNet model from MONAI, configured for binary classification
    ...
    ).to(rank)  # Move model to the appropriate GPU based on rank
    ...
    train_transforms = Compose([EnsureChannelFirst(channel_dim=3), transformations])
    validation_transforms = Compose([EnsureChannelFirst(channel_dim=3)])
    test_transforms = Compose([EnsureChannelFirst(channel_dim=3)])
